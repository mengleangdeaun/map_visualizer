...
type MapProps = {
  ...
  /** Language for map labels (e.g. 'km', 'en', 'zh') */
  language?: string;
...
const Map = forwardRef<MapRef, MapProps>(function Map(
  ...
  useEffect(() => {
    ...
    const map = new MapLibreGL.Map({
      ...
      // Use local font shaping for better Khmer support
      localIdeographFontFamily: "'Noto Sans Khmer', 'Kantumruy', 'Arial', sans-serif",
  ...
  useEffect(() => {
    ...
    const updateLanguage = () => {
      ...
      style.layers.forEach((layer) => {
        if (layer.type === "symbol" && layer.layout) {
          ...
          // 1. Update text-field for language
          if (textField && JSON.stringify(textField).includes("name")) {
            const langField = language ? `name:${language}` : "name";
          ...
          // 2. Force Khmer-compatible font stack
          const currentFont = layout["text-font"] || ["Open Sans Regular"];
          const newFontStack = Array.isArray(currentFont) ? [...currentFont] : [currentFont];
          
          if (!newFontStack.includes("Noto Sans Khmer Regular")) {
            // Add Khmer fonts to the beginning of the stack
            newFontStack.unshift("Noto Sans Khmer Regular", "Noto Sans Khmer Bold", "Kantumruy Pro Regular");
            mapInstance.setLayoutProperty(layer.id, "text-font", newFontStack);
          }
        }
      });
    };
    ...
    // Also handle style changes which reset layout properties
    mapInstance.on("styledata", updateLanguage);
    return () => {
      mapInstance.off("styledata", updateLanguage);
    };
  }, [mapInstance, isStyleLoaded, language]);
...
function PopupCloseButton({ onClick }: { onClick: () => void }) {
  return (
    <button
      ...
      className="focus-visible:ring-ring hover:bg-muted text-foreground absolute top-0.5 end-0.5 z-10 inline-flex size-5 cursor-pointer items-center justify-center rounded-sm transition-colors focus:outline-none focus-visible:ring-2"
    >
