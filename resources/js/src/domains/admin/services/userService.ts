...
export interface User {
    ...
    role: 'admin' | 'dispatcher' | 'hub_operator' | 'driver';
    name: string;
    phone: string;
    email: string | null;
...
export const userService = {
    getUsers: async (params: any) => {
        const response = await api.get<PaginatedResponse<User>>(`/admin/fleet/users`, {
            params
        });
        return response.data;
    },
    getUser: async (id: string) => {
        const response = await api.get<User>(`/admin/fleet/users/${id}`);
        return response.data;
    },
    createUser: async (data: FormData | Partial<User>) => {
        const response = await api.post('/admin/fleet/users', data);
        return response.data;
    },
    updateUser: async (id: string, data: FormData | Partial<User>) => {
        ...
        const response = await api.post(`/admin/fleet/users/${id}`, finalData);
        return response.data;
    },
    deleteUser: async (id: string) => {
        const response = await api.delete(`/admin/fleet/users/${id}`);
        return response.data;
    },
};
