...
import { Loader2, ShieldCheck, MapPin, Send, User as UserIcon } from 'lucide-react';
...
const getUserSchema = (isEditing: boolean) => z.object({
...
const UserForm = ({ open, onOpenChange, user }: UserFormProps) => {
    ...
    const form = useForm({
        defaultValues: {
            role: (user?.role || 'driver') as string,
            name: user?.name || '',
            phone: user?.phone || '',
            email: user?.email || '',
            password: '',
        ...
        onSubmit: async ({ value }) => {
            ...
            // Inject the implicit company_id
            if (companyId) {
                        ...
                        <div className="p-4 space-y-6">
                            ...
                            <div className="bg-muted/30 p-4 rounded-xl border border-dashed space-y-4">
                                <Label className="text-[10px] font-bold text-muted-foreground uppercase tracking-widest flex items-center gap-2">
                                    ...
                                    {t('base_hub', 'Base Hub Assignment')}
                                </Label>
                                ...
                                <form.Field
                                    ...
                                    children={(field) => (
                                        <div className="space-y-2 max-w-md">
                                            ...
                                            <SearchableSelect
                                                options={(hubsData?.data as Location[]) || []}
                                                value={field.state.value}
                                                onChange={(val) => field.handleChange(val || '')}
                                                isLoading={isLoadingHubs}
                                                placeholder={t('select_hub', 'Select base hub...')}
                                                searchPlaceholder={t('search_hub', 'Search hub...')}
                                                getOptionValue={(h: Location) => h.id}
                                                getOptionLabel={(h: Location) => h.name}
                                            />
                                        </div>
                                    )}
                                />
                                            ...
                                            <div className="space-y-2">
                                                ...
                                                >
                                                    ...
                                                    <SelectContent>
                                                        ...
                                                        <SelectItem value="dispatcher">{t('role_dispatcher', 'Dispatcher')}</SelectItem>
                                                        <SelectItem value="hub_operator">{t('role_hub_operator', 'Hub Operator')}</SelectItem>
                                                        <SelectItem value="driver">{t('role_driver', 'Driver')}</SelectItem>
                                                    </SelectContent>
                                                </Select>
                                                {field.state.meta.isTouched && field.state.meta.errors.length > 0 && (
                                                    <p className="text-xs text-destructive">{field.state.meta.errors[0]}</p>
                                            ...
                                            <div className="space-y-2">
                                                ...
                                                >
                                                    ...
                                                    <SelectContent>
                                                        ...
                                                        <SelectItem value="suspended">{t('status_suspended', 'Suspended')}</SelectItem>
                                                        <SelectItem value="inactive">{t('status_inactive', 'Inactive')}</SelectItem>
                                                    </SelectContent>
                                                </Select>
                                            </div>
                                        )}
