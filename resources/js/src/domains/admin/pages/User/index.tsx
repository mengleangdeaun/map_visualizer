...
import { Plus, Edit2, Trash2, User as UserIcon, RotateCw, Send } from 'lucide-react';
...
const UserPage = () => {
                ...
                return (
                    <div className="flex items-center gap-3">
                        ...
                        <div className="flex flex-col">
                            ...
                            <span className="text-[10px] font-bold text-muted-foreground font-mono leading-tight mt-0.5">
            ...
            cell: ({ row }) => {
                ...
                return (
                    <div className="flex flex-col">
                        {user.hub ? (
                            <>
                                <span className="text-sm font-semibold">{user.hub.name}</span>
                                <span className="text-[10px] text-muted-foreground font-mono">{user.hub.code}</span>
                            </>
                        ) : (
            ...
            cell: ({ row }) => {
                ...
                return (
                    <div className="flex flex-col gap-1">
                        ...
                        {user.role === 'driver' && user.telegram_chat_id && (
                            <span className="text-[9px] font-mono text-muted-foreground max-w-[150px] truncate bg-muted px-1 py-0.5 rounded leading-none w-fit">
                                Chat: {user.telegram_chat_id}
                                {user.telegram_topic_id && ` | Topic: ${user.telegram_topic_id}`}
                            </span>
                        )}
                    </div>
                );
    ...
    return (
        <div className="space-y-6">
            <PageHeader
                title={t('driver_management', 'Users & Drivers')}
                subtitle={t('manage_company_users_and_roles', 'Manage driver staff, dispatchers, hub operators, and administrative accounts.')}
                refreshAction={{
                    onClick: () => refetch(),
            ...
            <DeleteConfirmModal
                ...
                title={t('delete_user')}
                description={t('are_you_sure_you_want_to_delete_user', { name: userToDelete?.name, defaultValue: `Are you sure you want to delete user ${userToDelete?.name}?` })}
