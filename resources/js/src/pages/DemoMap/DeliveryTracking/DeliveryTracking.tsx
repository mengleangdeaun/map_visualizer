...
const DeliveryTracking = () => {
    const [viewport, setViewport] = useState({
        center: [104.9281, 11.5650] as [number, number],
        zoom: 14,
        pitch: 45,
        bearing: 0
    });
            ...
            >
                ...
                {pickup && dropoff && !isAnimating && status === 'Ready to Start' && (
                    <Button 
                        ...
                        className="bg-emerald-600 hover:bg-emerald-700 animate-in zoom-in duration-300"
                    >
            ...
            <div className="grid grid-cols-1 lg:grid-cols-4 gap-6 flex-1 min-h-0">
                <div className="lg:col-span-3 h-full relative rounded-xl border bg-card shadow-sm overflow-hidden">
                    ...
                    >
                            ...
                            <>
                                {routes.map((r, index) => {
                                    ...
                                    return (
                                         <MapRoute
                                            ...
                                            opacity={isSelected ? (isAnimating ? 0.25 : 1) : 0.15}
                                            dashArray={undefined}
                                            onClick={() => {
                                                if (!isAnimating) {
                                ...
                                {isAnimating && (
                                    <>
                                        {/* Ultra Outer Glow */}
                                        <MapRoute 
                                            coordinates={routes[selectedIndex].coordinates.slice(0, Math.floor(tracking.progress / (100 / (routes[selectedIndex].coordinates.length - 1))) + 1).concat(tracking.pos ? [tracking.pos] : [])} 
                                            color="#10b981" 
                                            width={16} 
                                            opacity={0.15} 
                                            blur={8}
                        ...
                        {pickup && (
                            <MapMarker longitude={pickup[0]} latitude={pickup[1]}>
                                <MarkerContent>
                                    ...
                                    <MarkerLabel position="top" className="bg-background px-2 py-0.5 rounded shadow-sm font-bold text-[10px]">PICKUP</MarkerLabel>
                                </MarkerContent>
                            </MapMarker>
                        )}
                        ...
                        {dropoff && (
                            <MapMarker longitude={dropoff[0]} latitude={dropoff[1]}>
                                <MarkerContent>
                                    ...
                                    <MarkerLabel position="top" className="bg-background px-2 py-0.5 rounded shadow-sm font-bold text-[10px]">DROPOFF</MarkerLabel>
                                </MarkerContent>
                            </MapMarker>
                        )}
                            ...
                            <MapMarker longitude={searchMarker[0]} latitude={searchMarker[1]}>
                                <MarkerContent>
                                    <div className="relative group">
                                        ...
                                        <MarkerLabel position="top" className="bg-background/95 backdrop-blur-sm px-3 py-1.5 rounded-lg shadow-xl border border-emerald-100 flex flex-col items-center gap-1.5 min-w-[120px]">
                                            <span className="text-[10px] font-bold text-foreground truncate max-w-[150px]">{searchLabel}</span>
                                            {!isAnimating && (
                                                <div className="flex gap-1 w-full">
                                                    {!pickup && (
                                                        <Button 
                                                            size="xs" 
                                                            className="flex-1 text-[8px] h-5 bg-emerald-600"
                                                            onClick={() => {
                        ...
                        {tracking.pos && (
                            <MapMarker 
                                longitude={tracking.pos[0]} 
                                latitude={tracking.pos[1]}
                            ...
                            >
                                <MarkerContent>
                                    <div className="relative group cursor-pointer">
                                        ...
                                        <div className="relative size-10 rounded-full bg-white border-2 border-emerald-500 shadow-2xl flex items-center justify-center transition-transform group-hover:scale-110">
                                            <div className="bg-emerald-500 p-1.5 rounded-full text-white">
                                                {vehicleType === 'car' && <Car className="size-5" />}
                                                {vehicleType === 'bike' && <Bike className="size-5" />}
                                                {vehicleType === 'tuk_tuk' && (
                                        ...
                                        {tracking.eta > 0 && (
                                            <div className="absolute bottom-full left-1/2 -translate-x-1/2 mb-2 whitespace-nowrap bg-emerald-600 text-white text-[10px] font-bold px-2 py-1 rounded shadow-lg pointer-events-none">
                    ...
                    {isAnimating && (
                        <div className="absolute bottom-4 left-4 p-4 rounded-xl border bg-background/95 backdrop-blur-md shadow-xl max-w-xs space-y-3 animate-in slide-in-from-bottom-4">
